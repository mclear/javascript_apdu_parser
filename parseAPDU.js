(() => {
    ...
    class ApduParser
    {
        static parseAPDU (apdu) {
            ...
            parsed.command = ApduParser._parse_cmd(apdu);
            parsed.response = ApduParser._parse_rsp(apdu);
        ...
        static _parse_cmd (apdu) {
            ...
            const {name, description} = commands.find(c => c.code === cmd.ins) || def;

            return Object.assign(cmd, {name, description});
        }

        static _parse_rsp (apdu) {
            const
                def = {type: '', description: 'NOT FOUND'},
                [sw1, sw2, ...data] = apdu.split(' '),
                rsp = {sw1, sw2, data};

            const {type, description} = responses.find(r => r.SW1 === sw1 && r.SW2 === sw2) || def;

            return Object.assign(rsp, {type, description});
        }
    }
